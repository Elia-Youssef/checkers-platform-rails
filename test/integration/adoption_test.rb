...
class AdoptionTest < ActionDispatch::IntegrationTest
  ...
  test "the adopted matches are listed in My games under the account, and name the user" do
    ...
    # One identity on both seats, before and after the adoption, so the cell names no
    # opponent either way; the computer's seat is the other identity and is named.
    browser.assert_select "tr#match-row-#{hotseat.id} .games__opponent", text: "Yourself, both seats"
