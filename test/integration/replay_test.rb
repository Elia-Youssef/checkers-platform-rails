...
class ReplayTest < ActionDispatch::IntegrationTest
  ...
  test "a middle ply shows the position after that many moves" do
    ...
    # The tint on the two squares of the replayed ply is in their names as well, so stepping
    # the replay tells a screen reader what moved (round-2 accessibility audit, finding M1).
    assert_equal "Square 11, empty, moved from here", board_squares[11]
    assert_equal "Square 15, Red man, moved here", board_squares[15]
  ...
  # ---- the ply sentence at the two ends (round-2 accessibility audit, finding L2) ----------

  # pluralize(total, "move") wrote "before any of the 0 moves" for a match nobody has played in
  # and "before any of the 1 move" for a match with one, so both ends of the range read wrongly.
  test "the ply sentence reads for a match with no moves, one move and many" do
    empty = Match.open_hotseat(guest_key: SecureRandom.urlsafe_base64(24))
    get match_replay_path(empty)

    assert_select ".replay__ply", text: "The starting position."

    one = Match.open_hotseat(guest_key: SecureRandom.urlsafe_base64(24))
    play(one, "11-15")
    get match_replay_path(one)

    assert_select ".replay__ply", text: "The starting position, before the only move."

    get match_replay_path(@match)
    assert_select ".replay__ply", text: "The starting position, before any of the 4 moves."

    get match_replay_path(@match, ply: 2)
    assert_select ".replay__ply", text: "After ply 2 of 4."
  end

  # ---- what the highlight says (round-2 accessibility audit, finding M1) -------------------

  # Stepping the replay is the whole point of the page, and the entry it is showing was marked
  # by a background colour alone, so a screen reader could not tell which one it was.
  test "the replayed ply is the current entry in the move list" do
    (0..4).each do |ply|
      get match_replay_path(@match, ply: ply)

      current = Nokogiri::HTML(response.body).css(".moves__move[aria-current]")

      assert_equal (ply.zero? ? [] : [ @rows[ply - 1].pdn ]), current.map(&:text),
        "ply #{ply} marks the wrong entry as current"
      assert_equal highlighted_moves, current.map(&:text),
        "the marked entry and the current entry are not the same entry at ply #{ply}"
      assert(current.all? { |node| node["aria-current"] == "true" })
    end
  end
