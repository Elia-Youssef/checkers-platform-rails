...
class AdoptionTest < ApplicationSystemTestCase
  test "a guest who signs up keeps the games they started in this browser" do
    ...
    click_link "My games"
    assert_selector "tr#match-row-#{hotseat.id} .games__opponent", text: "Guest"
    ...
    # Both games are the account's now, and are named after it.
    click_link "My games"
    assert_selector "tr#match-row-#{hotseat.id} .games__opponent", text: "Newcomer"
    assert_selector "tr#match-row-#{computer.id} .games__opponent", text: "Computer (Easy)"
