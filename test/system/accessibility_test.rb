...
class AccessibilityTest < ApplicationSystemTestCase
  ...
  # Chromium's own forced-colours emulation, the call the round-2 audit used. Passing no
  # features puts the browser back, which matters because Capybara keeps this browser for the
  # tests that follow.
  def emulate_forced_colours(active)
  ...
  # The colours the browser actually paints on the board, whatever mode it is in.
  def board_colours
    page.evaluate_script(<<~JS)
      (() => {
        const style = (el) => el ? getComputedStyle(el) : null
        const dark = document.querySelector("#match-board button[data-square='9']")
        const light = document.querySelector(".board__cell--light")
        const dot = document.querySelector(".square--target .square__dot")
        const tinted = document.querySelector("#match-board button.square--last-move")
        return {
          dark: style(dark).backgroundColor,
          light: style(light).backgroundColor,
          dot: dot ? style(dot).backgroundColor : "no destination",
          dotDisplay: dot ? style(dot).display : "no destination",
          tinted: tinted ? style(tinted).backgroundColor : "no tint"
        }
      })()
    JS
  end

  # What the page's live region is saying, which is what a screen reader would have read out.
  def announcement
    page.evaluate_script(<<~JS).to_s.strip
      (() => {
  ...
  # The announcer writes when Stimulus connects the replaced status fragment, a beat after the
  # markup arrives, so this waits for the sentence rather than reading once.
  def assert_announced(text)
    Timeout.timeout(5) { sleep 0.02 until announcement.include?(text) }
    assert_includes announcement, text
  rescue Timeout::Error
    flunk "the live region never said #{text.inspect}; it holds #{announcement.inspect}"
  end
  ...
  # ---- rubric 22, 32 and 39: what the highlight and the tint say (round-2 audit, M1) --------

  # The marked move and the two marked squares were a background colour and a ring and nothing
  # else, so a screen reader was told neither which entry the list is pointing at nor which
  # squares just changed. Both are now in the markup: aria-current on the entry, and the two
  # ends of the move in the squares' own names.
  test "the highlighted move and the tinted squares say so, and not only in colour" do
    match = start_hotseat
    ...
    assert_selector ".moves__move[aria-current='true']", text: "11-15", count: 1
    assert_equal "Square 11, empty, moved from here", square(11)["aria-label"]
    assert_equal "Square 15, Red man, moved here", square(15)["aria-label"]
    assert_equal "Square 22, White man", square(22)["aria-label"],
      "a square that had nothing to do with the last move says nothing about it"

    # And both marks move on with the game: exactly one entry is current and exactly two
    # squares carry the words, whatever the ply.
    square(22).send_keys(:enter)
    square(18).send_keys(:space)
    assert_selector ".moves__move--latest", text: "22-18"

    assert_selector ".moves__move[aria-current='true']", text: "22-18", count: 1
    assert_equal "Square 22, empty, moved from here", square(22)["aria-label"]
    assert_equal "Square 18, White man, moved here", square(18)["aria-label"]
    assert_equal "Square 15, Red man", square(15)["aria-label"]
    assert_equal 2, all("##{Match::BOARD_ID} button[aria-label*=' moved ']").length
    assert_equal 2, match.reload.moves.count
  end

  # ---- Windows High Contrast and other forced-colours modes (round-2 audit, M3) -------------

  # Forced colours repaints every background with the user's own, which turned the board into a
  # plain white field and made the destination dots vanish: measured at rgb(255, 255, 255) for
  # the dark square, the light square and the dot alike. The board draws its own colours now
  # (forced-color-adjust in board.css), and this checks it in the browser under Chromium's own
  # emulation rather than by reading the stylesheet.
  test "the board keeps its squares, its dots and its tint in forced-colours mode" do
    match = start_hotseat
    ...
    emulate_forced_colours(true)
    assert page.evaluate_script("matchMedia('(forced-colors: active)').matches"),
      "the forced-colours emulation did not take, so nothing below means anything"

    painted = board_colours
    assert_not_equal painted["light"], painted["dark"],
      "both kinds of square are #{painted["dark"]}, so there is no checkerboard"
    assert_not_equal painted["dark"], painted["dot"],
      "the destination dot is the same colour as the square it sits on (#{painted["dot"]})"
    assert_not_equal painted["dark"], painted["tinted"],
      "the last move's squares are not distinguishable from the rest"
    assert_equal "block", painted["dotDisplay"], "the destination dot is not drawn at all"
  ensure
    emulate_forced_colours(false)
  end
  ...
  # ---- what a screen reader is told when the opponent acts (round-2 audit, M2) --------------

  # Every accepted action replaces the status fragment, and the role="status" paragraph is
  # inside it. A live region that is replaced is a new node, and a reader announces changes to
  # the region it was already watching, so both players were told nothing when a move arrived:
  # the acting browser was covered only by accident, because moving focus reads the new
  # square's label out. The page carries one region that nothing replaces, and the announcer
  # controller writes the server's sentence into it after every replacement.
  test "a live update is announced in a region that the update does not replace" do
    ...
    Capybara.using_session(:grace) do
      ...
      # One polite region, outside every fragment a stream replaces, and silent on arrival:
      # the sentence on the page has just been read, so repeating it would be noise.
      assert_selector "#match-announcer[aria-live='polite'][role='status']", visible: :all
      assert_equal "", announcement
      assert_empty page.all("##{Match::STATUS_ID} #match-announcer", visible: :all),
        "the live region is inside the fragment that gets replaced"
      page.execute_script("window.watched = document.getElementById('match-announcer')")
    end
    ...
    Capybara.using_session(:ada) do
      ...
      assert_selector ".moves__move--latest", text: "11-15"
      assert_announced "White to move"
    end

    Capybara.using_session(:grace) do
      # It arrived over the socket, into the element the reader was already watching.
      assert_selector ".moves__move--latest", text: "11-15"
      assert_announced "White to move"
      assert page.evaluate_script(
        "window.watched === document.getElementById('match-announcer')"
      ), "the live region was replaced, which is what silences a screen reader"

      # And the other way: Grace replies and Ada hears it.
